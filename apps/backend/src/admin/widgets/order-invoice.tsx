...
/** The store's "from" contact line, from editable settings. */
function fromLine(store: Store): string {
  return [store.phone && `Phone ${store.phone}`, store.whatsapp && `WhatsApp ${store.whatsapp}`, store.email]
    .filter(Boolean)
    .map(esc)
    .join(" · ")
}
...
async function loadStore(): Promise<Store> {
  const store: Store = { address: "", email: "", phone: "", whatsapp: "" }
  try {
